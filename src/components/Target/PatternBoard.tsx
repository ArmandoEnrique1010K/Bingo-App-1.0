...
export default function PatternBoard({targetPositions}: PatternBoardProps) {
      ...
      {rows.map((row, rowIndex) => (
        ...
        >
          {row.map((num, colIndex) => (
            <div
              key={colIndex}
              className={`size-8 border-solid border-2 border-gray-700 flex items-center justify-center ${verifyTargets.includes(num) ? "bg-cyan-600" : "bg-gray-600"}`}  
            //   style={{
            //     width: '40px',
            //     height: '40px',
